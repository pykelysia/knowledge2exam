...
试卷采用整卷直写：agent 用 edit_file 直接书写 output/paper.md（无题目 JSON
中间产物、无渲染时合并），render_paper 只负责把该文件渲染为 PDF。生成与修订
共用同一套工具，差异只体现在系统提示词与任务指令上。
...
# 试卷唯一产物：agent 直写，渲染与前端预览共用同一份文件
...
def finalize(
    ...
    """汇总运行产物：蓝图来自 ctx.todos，放弃题从蓝图标记提取。"""
    abandoned = sorted(t.seq for t in ctx.todos if t.knowledge_point.startswith(ABANDONED_PREFIX))
    return ExamResult(
        intent=intent,
        plan_items=sorted(ctx.todos, key=lambda t: t.seq),
        abandoned_seqs=abandoned,
        summary=summary,
...
class ReadFileArgs(BaseModel):
    ...
    path: str = Field(description="工作区相对路径，如 agent/materials/past_paper_01.md")


class EditFileArgs(BaseModel):
    """edit_file 的参数。"""

    path: str = Field(description="工作区相对路径，试卷为 output/paper.md")
    old_string: str = Field(
        default="",
        description="要替换的原文；创建新文件时传空字符串",
    )
...
def _todo_write_tool(ctx: AgentContext) -> StructuredTool:
    async def todo_write(todos: list[TodoItem]) -> str:
        ...
        await ctx.hooks.fire_progress(done, len(todos))
...
def _read_file_tool(ctx: AgentContext) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        coroutine=read_file,
        name="read_file",
        description="读取工作区中的文件（用户材料、试卷 output/paper.md 等），返回文本内容。",
...
def _edit_file_tool(ctx: AgentContext) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        name="edit_file",
        description=(
            "编辑工作区文件，也是书写试卷的工具。创建文件：old_string 传空字符串"
            "（试卷首次创建即如此）；修改：old_string 传文件中唯一出现的原文片段，"
            "替换为 new_string（可用于逐题追加或修正既有内容）。"
            "试卷必须写入 output/paper.md。"
        ),
...
async def _render_paper_once(ctx: AgentContext) -> int:
    """执行一次 PDF 渲染：读 output/paper.md → 清洗 → 渲染 → 写 output/paper.pdf。

    磁盘上的 paper.md 始终保持 agent 原稿（清洗只作用于喂给 pandoc 的字节，
    不回写文件），前端预览与 md 交付物与之完全一致。
    返回 md 字符数；失败抛异常，由调用方分类处理。
    """
    await ctx.hooks.fire_render_start()
    md_text = await ctx.workspace.read(PAPER_PATH)
    md_bytes = sanitize_markdown(md_text).encode("utf-8")
    ...
    pdf = await renderer.render_markdown(md_bytes, title=f"试卷（{ctx.duration_minutes} 分钟）")
    await ctx.storage.put(f"jobs/{ctx.job_id}/{PDF_PATH}", pdf)
...
def _render_paper_tool(ctx: AgentContext) -> StructuredTool:
    async def render_paper() -> str:
        ...
        if ctx.render_status == "md_only":
            return (
                "渲染此前已停止（PDF 未生成），试卷 output/paper.md 仍可交付。"
                "请直接输出总结并结束，不要再调用 render_paper。"
            )
        if not await ctx.workspace.exists(PAPER_PATH):
            return (
                f"尚未创建试卷：{PAPER_PATH} 不存在。"
                "请先用 edit_file 把试卷内容写入 output/paper.md 后再渲染。"
            )

        try:
            chars = await _render_paper_once(ctx)
        except (RenderEnvironmentError, FileNotFoundError) as exc:
            # 环境性错误：修改内容无法修复，跳过重试直接交付 md
            ctx.render_status = "md_only"
            ctx.render_error = str(exc)
            await ctx.hooks.fire_warning(f"渲染环境异常，跳过重试交付 md：{exc}")
            return (
                f"渲染环境异常（{exc}），无法通过修改试卷修复，已停止重试。"
                "试卷 output/paper.md 已就绪，请直接输出总结并结束，不要再调用 render_paper。"
            )
        except Exception as exc:
            ...
            return (
                ...
                f"用 edit_file 修正 {PAPER_PATH} 中的相关内容后重新调用 render_paper。"
            )
        return (
            f"试卷渲染成功：{PAPER_PATH} 与 {PDF_PATH} 已生成（{chars} 字符）。"
            "请直接输出总结并结束。"
        )
    ...
    return StructuredTool.from_function(
        ...
        name="render_paper",
        description=(
            "把整份试卷（output/paper.md）渲染为 PDF 交付产物。所有题目完成后必须调用本工具，"
            "渲染成功（或工具明确告知已放弃渲染）后才能输出总结结束。"
            "失败时会返回 pandoc 错误信息，需用 edit_file 修正试卷后重试。"
        ),
    )
